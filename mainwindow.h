#ifndef MAINWINDOW_H
#define MAINWINDOW_H

#include <QMainWindow>

#include "colapasajeros.h"
#include "listacequipaje.h"
#include "listacmantenimiento.h"
#include "listadaviones.h"
#include "listadescritorios.h"
#include "piladocumentos.h"
#include <iostream>
#include <string>

namespace Ui {
class MainWindow;
}

class MainWindow : public QMainWindow
{
    Q_OBJECT

public:
    explicit MainWindow(QWidget *parent = 0);
    ~MainWindow();

    int turn;

    listaDaviones* listaAviones ;

    colapasajeros* desabordaje;


    colapasajeros* colapasa;
    listaCequipaje* equipaje;
    listaCMantenimiento* mantenimiento;
    listaDescritorios* escritorios;
    piladocumentos* documentos;

    listaCMantenimiento * estaciones;



private slots:
    void on_pushButton_clicked();

    void on_pushButton_2_clicked();

    void on_pushButton_3_clicked();

    void on_pushButton_4_clicked();

    void on_pushButton_5_clicked();

    void on_pushButton_6_clicked();

    void on_generateButton_clicked();

    void on_turnButton_clicked();

private:
    Ui::MainWindow *ui;
};

#endif // MAINWINDOW_H
